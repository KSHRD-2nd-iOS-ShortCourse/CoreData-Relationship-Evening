...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let categoryEntity : NSEntityDescription? = NSEntityDescription.entityForName("Category", inManagedObjectContext: context)
        
        let postEntity : NSEntityDescription? = NSEntityDescription.entityForName("Post", inManagedObjectContext: context)
        
        // Check
        if categoryEntity != nil && postEntity != nil {
            
            // Create Category from context
            let category1 : Category = Category(entity: categoryEntity!, insertIntoManagedObjectContext: context)
            ...
            for i in 1...5{
                let currentPost : Post = Post(entity: postEntity!, insertIntoManagedObjectContext: context)
            ...
            let category2 : Category = Category(entity: categoryEntity!, insertIntoManagedObjectContext: context)
            ...
            let category3 : Category = Category(entity: categoryEntity!, insertIntoManagedObjectContext: context)
            category3.name = "Book"
        ...
        // Create entity
        let nsFetch = NSFetchRequest(entityName: "Category")
        do{
            // Read All Category
            if let categoryResults = try context.executeFetchRequest(nsFetch) as? [Category] {
